...
docname: draft-liao-ace-est-c509-02
abbrev: EST-C509
...
- For non-KEM-only key types, the C509 CSR is typically pre-provisioned as an opaque binary blob by the device manufacturer or a provisioning tool; the EST client sends it verbatim as the POST body of `sen` (simple enroll) or `sren` (simple reenroll) without interpreting its contents.
...
In EST over CoAP/DTLS, the capabilities is retrieved by sending a GET method to the EST server (as in {{Section 4.1 of RFC9148}}) (TODO: the TBD will be replaced with the real value once the Content-Formats are assigned in {{I-D.ietf-cose-cbor-encoded-cert}} and {{I-D.liao-cose-c509-revocation}}):
...
The operations in this document follow the same URI path structure defined in {{RFC7030, Section 3.2.2}} for HTTPS and the corresponding secure CoAP mapping defined in {{RFC9148}}.  Retrieval operations (`cacaps`, `cacerts`, `crli`, `crl`) use HTTP GET, and (`crts`, `crli`, `crl`) use CoAP GET:
...
Enrollment operations (`kemc`, `simpleenroll` / `sen`, `simplereenroll` / `sren`, `serverkeygen` / `skc`) use HTTP POST or CoAP POST:
...
The optional `<label>` path segment follows {{RFC7030, Section 3.2.2}} for HTTPS and the equivalent path structure in {{RFC9148}} for CoAP: an EST server MAY use an additional path segment before the operation name to distinguish services for multiple CAs or certificate profiles.
...
The mechanism for delayed responses specified in {{RFC9148, Section 4.7}} applies for CoAP in this document. And the mechanism with retry-later for the operarions `simpleenroll` / `sen` and `simplereenroll` / `sren` specified in {{RFC7030, Section 4.2.3}} applies for HTTP in this document.
...
This CRAttribute MAY be included in a `sren` request to change the `Subject` field and/or the `SubjectAltName` extension in the newly generated certificate.

## Proof of Possession

`simpleenroll` / sen` and `simplereenroll` / `sren` MUST verify the PoP signature in the C509 CSR before issuing a certificate.  The `simplereenroll` / `skc` operation does not require PoP verification because the EST server generates the key pair itself.
...
- Challenge issuance: Upon receipt of a KEM public key (`C509PublicKey`) in the `kemc` operation, an EST server returns a CBOR-encoded KEM challenge object (`C509KemChall`) with media type `application/cbor`.
...
- Client decapsulation and response: The client decapsulates `encapsulation` to recover the one-time secret key `K`.  The client computes a MAC value over the CBOR-encoded `TBSCertificationRequest` with the key `K`.  The client then submits a follow-up operation as usual.

- Server verification: The server computes the `keyId`, retrieves the challenge state for that `keyId`, and verifies that the state is still within its validity period.  The server then verifies the received `mac` against the saved `TBSCertificationRequest`.  The server proceeds with certificate issuance as for signature-based PoP.  If verification fails or the challenge has expired, the server MUST reject the request.
...
This operation is only related to EST over HTTPS, but not EST over CoAP/DTLS.
...
- Media type: `text/plain; charset=utf-8` (HTTP)
...
Successful `att` responses MAY include caching metadata as specified in {{response-cache}}.
...
- `Accept: application/cose-c509-cert` (HTTP) / `Accept=TBD` (CoAP): a single certificate

- `Accept: application/cose-c509; usage=chain` (HTTP) / `Accept=TBD` (CoAP): certificate chain

- No Accept or `Accept: application/cose-c509` (HTTP) / `Accept=TBD` (CoAP): certificate set (unsorted certificates)
...
A client that receives a `C509KemChall` uses the recovered one-time secret key to produce the PoP MAC and then proceeds with a normal enrollment request (`sen` or `sren`) including the computed MAC in the request (see {{pop-kem}} for the PoP flow). The EST server verifies the MAC using the stored challenge state and issues the certificate on success.
...
On success, the EST server returns a HTTP 200 / CoAP 2.04 response with:
...
An authenticated EST client sends a POST request containing a C509 CSR.  The `subjectPublicKeyAlgorithm` in the C509 CSR SHOULD be set to `empty-publickey` ({{empty-publickey}}) and `subjectPublicKey` SHOULD be an empty byte string (`h''`), because the key pair is generated by the EST server.  The `signatureAlgorithm` SHOULD be the `id-alg-unsigned` integer code and `signatureValue` SHOULD be a zero-length byte string.  EST servers MUST accept C509 CSRs that use `empty-publickey` and `id-alg-unsigned` for `skc` and MUST NOT verify a PoP signature in this case.
...
| Parameters | N/A |
| DER | N/A |
| Comments | Exclusively for use in `subjectPublicKeyAlgorithm` of a `TBSCertificationRequest` for server-side key generation (`skc`).  MUST NOT appear in C509 certificates. |
...
# Message Flow Diagrams over HTTPS {#flows-http}

## cacerts {#flow-crts-http}
...
EST Client                                    EST Server
  ...
  |                                               |
~~~
{: #fig-crli-http title="HTTP crli message flow"}
...
EST Client                                   EST Server
  ...
  |                                               |
~~~
{: #fig-crl-http title="HTTP crl message flow"}
...
|                                               |
~~~
{: #fig-kemc-http title="HTTP kemc message flow"}
...
EST Client                                 EST Server
  ...
  |                                               |
~~~
{: #fig-sen-http title="HTTP simpleenroll message flow"}
...
|                                               |
~~~
{: #fig-sren-http title="HTTP simplereenroll message flow"}
...
EST Client                                  EST Server
  ...
  |                                               |
~~~
{: #fig-skc-http title="HTTP serverkeygen message flow"}

# Message Flow Diagrams over CoAP/DTLS {#flows-coap}
...
TBD
~~~
{: #fig-crts-coap title="CoAP crts message flow"}
...
TBD
~~~
{: #fig-crli-coap title="CoAP crli message flow"}
...
TBD
~~~
{: #fig-crl-coap title="CoAP crl message flow"}
...
TBD
~~~
{: #fig-kemc-coap title="CoAP kemc message flow"}
...
TBD
~~~
{: #fig-sen-coap title="CoAP sen message flow"}
...
TBD
~~~
{: #fig-sren-coap title="CoAP sren message flow"}
...
TBD
~~~
{: #fig-skc-coap title="CoAP skc message flow"}
